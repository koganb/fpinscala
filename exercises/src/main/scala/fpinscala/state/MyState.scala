...
object RNG {
    ...
    def mysequence[A](fs: List[Rand[A]]): Rand[List[A]] = rng => fs.foldRight(List[A](), rng)(
    ...
    def intsViaSequence(count: Int)(rng: RNG): (List[Int], RNG) = {
        mysequence(List.fill(count)(int))(rng)
    }
...
case class MyState[S, +A](run: S => (A, S)) {
    def map[B](f: A => B): MyState[S, B] = MyState((s: S) => {
        val res: (A, S) = run(s)
        (f(res._1), res._2)
    })

    def map2[B, C](sb: MyState[S, B])(f: (A, B) => C): MyState[S, C] = MyState((s: S) => {
        val res: (A, S) = run(s)
        val res1: (B, S) = sb.run(res._2)
        (f(res._1, res1._1), res._2)
    })

    def flatMap[B](f: A => MyState[S, B]): MyState[S, B] = MyState((s: S) => {
        val res: (A, S) = run(s)
        f(res._1).run(res._2)
    }
    )
...
object MyState {
    ...
    def mysequence[S, A](fs: List[MyState[S, A]]): MyState[S, List[A]] = MyState(s => fs.foldLeft(List[A](), s)(
        (b: (List[A], S), a: MyState[S, A]) => {
    ...
    def simulateMachine(inputs: List[Input]): MyState[Machine, (Int, Int)] = {
        mysequence(inputs.map { input => MyState(
            (machine: Machine) => input match {
                case Coin if (machine.candies > 0 && machine.locked) => ((machine.candies, machine.coins + 1), Machine(false, machine.candies, machine.coins + 1))
                case Turn if (machine.candies > 0 && !machine.locked) => ((machine.candies - 1, machine.coins), Machine(true, machine.candies - 1, machine.coins))
...
object Main extends App {
    ...
    private val state: ((Int, Int), Machine) = MyState.simulateMachine(List(Coin, Turn, Coin, Turn, Coin, Turn, Coin, Turn)).run(Machine(true, 5, 10))
    println(state)
